...
<?php
	...
	// 1 - kien thuc - /wp-admin/term.php?taxonomy=category&tag_ID=1&post_type=post
	// 117 - goc tu van - /wp-admin/term.php?taxonomy=category&tag_ID=117&post_type=post
	// 118 - san pham - /wp-admin/term.php?taxonomy=category&tag_ID=118&post_type=post
		...
		<?php if($id_cat != 140){ ?>
	 		<div id="columns_left" class="col-xs-12 col-sm-12 col-md-9 col-lg-9 vietnt-category khac-140">
				...
				<div class="overview col-xs-12 col-sm-12 col-md-12 col-lg-12">
					 <?php
					 	if($id_cat != 118) {
							...
							if($id_cat == 179 || $id_cat == 180 || $id_cat == 181 || $id_cat == 182) {

							}
							else {
						?>
						<div class="shop-search">
							<div class="excerptcontent col-xs-12 col-sm-12 col-md-12 col-lg-12">
								...
								<div class="form-inline">
									<div class="form-group">
										...
										<select id="chon_danh_muc" class="form-control">
											<?php foreach($this_category1 as $term) { echo $term->cat_ID;?>
											<?php $excludedIds = [264, 268, 261, 193, 186, 143, 232, 137];
											if (!in_array($term->cat_ID, $excludedIds)) {
											?>
											<option <?php echo ($term->cat_ID == $id_cat) ? 'selected' : '' ?> value="<?php echo get_category_link($term->cat_ID);?>"><?php echo $term->name;?></option>
											<?php } } ?>
										</select>
									</div>
								</div>
				...
				if($id_cat == 1 || $id_cat == 117 || $id_cat == 118) {
					...
					if($id_cat == 1) { ?>
						...
						<?php
							...
							<div class="view-more-cat cat-id-1">Xem đầy đủ nội dung</div>
						</div>
					<?php
					}
					// Goc tu van
				...
				<div class="right_cat col-xs-12 col-sm-12 col-md-6 col-lg-6">
					...
					if($id_cat == 1 || $id_cat == 117 || $id_cat == 118) {
						if($id_cat == 1) {
							$cat_args = array(
                                ...
                                'hide_empty' => 0,
                                'orderby'    => 'include',
                                'include'    => array( 231, 391, 238, 260, 468, 399, 396, 397, 398, 376 )
                            );
							...
							<div class="title_category" data-catid="<?php echo $term->cat_ID;?>">
								<?php
								$args = array(
									'post_type' => 'post',
									'cat' => $term->cat_ID,
					...
					?>
						<?php
							if($id_cat == 14444444){ //ban dau la 144 VietNT added on 12/04/2017
								$args = array(
									'post_type' => 'page',
									/*'showposts' => -1,*/
									'meta_query' => array(
					...
					<div class="row box-product-expand">
						<?php
							$args = array(
								...
								'cat' => $id_cat,
								'meta_query' => array(
									array('key' => 'style_product','value' => 2,),
									array('key' => 'hien_thi','value' => 1,),
								),
							);
							query_posts($args);
							$countpost = $wp_query->found_posts;
