	...
	<div class="container">
		...
		<div id="columns_left" class="col-xs-12 col-sm-12 col-md-9 col-lg-9">
			<div class="form_tuvan">
				<h3>Chuyên đề tư vấn bữa ăn hợp lý</h3>
				Để tính toán lượng dinh dưỡng thực tế từ bữa ăn của bạn xin vui lòng điền các thông tin sau:
				Tham khảo mẫu tư vấn bữa ăn hợp lý. Xem tại <span class="bieu_mau"><a href="/bieu-mau-ket-qua-chuyen-de-tu-van-bua-an-hop-ly.html" target="_blank">Biểu mẫu kết quả</a></span>

				<form accept-charset="utf-8" action="/thanh-toan-bua-an" method="post">
				...
				<ul>
					<li class="cols-xs-12 col-sm-12 col-md-12 col-lg-12" style="text-align: center;"><input id="guithongtinbuaan" name="guithongtinbuaan" type="submit" value="Gửi thông tin" /></li>
				</ul>
				</form></div>	
		</div>
...
$subject = "[NEW - ] Chuyên đề Tư vấn bữa ăn hợp lý ".$blogtime;
if($sex == 1){ 
	echo $sex_gt = "Nam";
}else{ 
	echo $sex_gt = "Nữ";
}
$message = "Thông tin khách chuyên đề tư vấn bữa ăn hợp lý \n Họ tên: ". $hoten ." \n Tuổi: ". $tuoi ."\n Giới tính: ".$sex_gt." \n Email: ".$myemail;
$headers = "";
$attachments = "";
$regex = "/([a-z0-9_]+|[a-z0-9_]+\.[a-z0-9_]+)@(([a-z0-9]|[a-z0-9]+\.[a-z0-9]+)+\.([a-z]{2,4}))/i"; */
/*
if(isset($_POST['guithongtinbuaan'])){
		if(!preg_match($regex, $myemail)){ 
			...
			<?php
				...
				$query_dinhduong = $wpdb->query("INSERT INTO xtl_bua_an values(
					'','$hoten','$tuoi','$sex','$weight','$height','$run','$bua_sang_t2','$bua_sang_t3','$bua_sang_t4','$bua_sang_t5','$bua_sang_t6', '$bua_sang_t7','$bua_sang_cn','$bua_trua_t2','$bua_trua_t3','$bua_trua_t4','$bua_trua_t5','$bua_trua_t6',
					...
					'$myemail','$payments','$create_date','$modify_date','$status')");
				if($query_dinhduong){
					...
					?>
						<script type="text/javascript" language="javascript">
							...
							window.location.replace("<?php echo get_bloginfo('url');?>/thong-bao-so-bill?emailbuaan=<?php echo $myemail;?>");
						</script>
					<?php
