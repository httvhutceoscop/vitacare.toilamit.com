 ...
 */
/*
Plugin Name: Image Upload Widget 2
...
class image_widget_2 extends WP_Widget {
...
public function __construct() {
		parent::__construct(
	 		'image_widget_2', // Base ID
			'Image Upload Widget 2', // Name
 
			array( 'description' => __( 'A widget to upload image 2', 'iuw' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$txtcontent = apply_filters( 'widget_txtcontent', $instance['txtcontent'] );
		$image_uri = apply_filters( 'widget_image_uri', $instance['image_uri'] );
		
		echo $before_widget; ?>
        	
			<p><?php echo $instance['name'];?></p>
        	<span><?php echo $instance['txtcontent'];?></span>
        	<a class="read-more" href="<?php echo esc_url($instance['link']); ?>">Read more...</a>
    <?php
		echo $after_widget;
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['txtcontent'] = ( ! empty( $new_instance['txtcontent'] ) ) ? strip_tags( $new_instance['txtcontent'] ) : '';
	...
	public function form( $instance ) {
		...
		if ( isset( $instance[ 'txtcontent' ] ) ) {
			$txtcontent = $instance[ 'txtcontent' ];
		}
		else {
			$txtcontent = __( 'New Content', 'iuw' );
		}
    ...
    <p>
      <label for="<?php echo $this->get_field_id('txtcontent'); ?>"><?php _e('Content', 'iuw'); ?></label><br />
      <textarea name="<?php echo $this->get_field_name('txtcontent'); ?>" id="<?php echo $this->get_field_id('txtcontent'); ?>" value="<?php echo $txtcontent; ?>" rows="16" cols="20" class="widefat" /></textarea>
    </p>
    <p>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "image_widget_2" );' ) );
function iuw_wdScript_2(){
  wp_enqueue_media();
  wp_enqueue_script('adsScript', plugins_url( '/js/image-upload-widget-2.js' , __FILE__ ));
}
add_action('admin_enqueue_scripts', 'iuw_wdScript_2');