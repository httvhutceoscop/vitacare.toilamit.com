	...
	<div class="container">
		 ...
		 <div id="columns_left" class="col-xs-12 col-sm-12 col-md-9 col-lg-9">
			<div class="form_tuvan">
				<h3>Chuyên đề tính dinh dưỡng</h3>
				Để tính toán lượng dinh dưỡng thực tế từ bữa ăn của bạn xin vui lòng điền các thông tin sau:
				Tham khảo mẫu sau khi tính toán lượng dinh dưỡng của bạn. Xem tại <span class="bieu_mau"><a href="/bieu-mau-ket-qua-chuyen-de-tinh-dinh-duong.html" target="_blank">Biểu mẫu kết quả</a></span>

				<form accept-charset="utf-8" action="/thanh-toan-dinh-duong" method="post">
				<ul class="info">
				...
				<ul>
					<li class="cols-xs-12 col-sm-12 col-md-12 col-lg-12" style="text-align: center;"><input id="guithongtindinhduong" type="submit" name="guithongtindinhduong" value="Gửi thông tin" /></li>
				</ul>
				</form></div>
		</div>
...
$subject = "[NEW - ] Chuyên đề tính dinh dưỡng ".$blogtime;
if($sex == 1){ 
	echo $sex_gt = "Nam";
}else{ 
	echo $sex_gt = "Nữ";
...
if(isset($_POST['guithongtindinhduong'])){
		if(!preg_match($regex, $myemail)){ 
			...
			<?php
				...
				$query_dinhduong = $wpdb->query("INSERT INTO xtl_dinh_duong values(
				...
				if($query_dinhduong){
					...
					?>
						<script type="text/javascript" language="javascript">
							...
							window.location.replace("<?php echo get_bloginfo('url');?>/thong-bao-so-bill?emaildinhduong=<?php echo $myemail;?>");
						</script>
					<?php
